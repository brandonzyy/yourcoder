...
describe("findPluginEntry", () => {
  ...
  test("returns unpinned for bare package name", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-plugin"] }))
  ...
  test("returns unpinned for latest dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-plugin@latest"] }))
  ...
  test("returns unpinned for beta dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-plugin@beta"] }))
  ...
  test("returns pinned for explicit semver", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["opencode-plugin@3.5.2"] }))
