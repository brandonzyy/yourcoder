...
25 tools registered via `createToolRegistry()`. Two patterns: factory functions (`createXXXTool`) for 18 tools, direct `ToolDefinition` for 7 (LSP + interactive_bash).

Note: The `skill` tool is now provided by the native tool system (`src/tool/skill.ts`) and is no longer part of the oh-my-openagent plugin tools.
