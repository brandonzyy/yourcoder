...
describe("AGENT_MODEL_REQUIREMENTS", () => {
  ...
  test("sisyphus prefers opus then kimi", () => {
    const req = AGENT_MODEL_REQUIREMENTS.sisyphus
  ...
  test("librarian and manon-explorer share the lightweight search chain", () => {
    expect(AGENT_MODEL_REQUIREMENTS.librarian.fallbackChain).toEqual([
      { providers: ["opencode"], model: "claude-haiku-4-5" },
      { providers: ["opencode"], model: "glm-4.7-fp8" },
    ])
    expect(AGENT_MODEL_REQUIREMENTS["manon-explorer"].fallbackChain).toEqual(
      AGENT_MODEL_REQUIREMENTS.librarian.fallbackChain,
    )
  })
})
