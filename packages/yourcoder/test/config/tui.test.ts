...
afterEach(async () => {
  delete process.env.YC_CONFIG
  delete process.env.YC_TUI_CONFIG
...
test("project config takes precedence over YC_TUI_CONFIG (matches YC_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.YC_TUI_CONFIG = custom
    },
  })
...
test("YC_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from YC_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.YC_CONFIG = path.join(customDir, "opencode.json")
    },
  })
