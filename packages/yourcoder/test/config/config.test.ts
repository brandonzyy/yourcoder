...
const managedConfigDir = process.env.YC_TEST_MANAGED_CONFIG_DIR!
...
test("does not try to install dependencies in read-only YC_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.YC_CONFIG_DIR
  process.env.YC_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.YC_CONFIG_DIR
    else process.env.YC_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable YC_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
// Note: preload.ts sets YC_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("YC_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["YC_DISABLE_PROJECT_CONFIG"]
    process.env["YC_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["YC_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["YC_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["YC_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["YC_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["YC_CONFIG_DIR"]
      process.env["YC_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["YC_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["YC_CONFIG_DIR"]
      } else {
        process.env["YC_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("YC_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["YC_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["YC_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["YC_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["YC_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from YC_CONFIG_DIR, not project
...
describe("YC_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in YC_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["YC_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["YC_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["YC_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["YC_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in YC_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["YC_CONFIG_CONTENT"]
