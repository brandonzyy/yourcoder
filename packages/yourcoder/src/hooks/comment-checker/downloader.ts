...
/**
 * Get the cache directory for yourcoder-plugin binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "yourcoder-plugin", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[opencode] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[opencode] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[opencode] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[opencode] Comment checking disabled.`)
    return null
  }
}
