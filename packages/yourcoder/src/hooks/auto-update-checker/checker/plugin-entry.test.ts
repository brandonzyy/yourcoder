...
describe("findPluginEntry", () => {
  ...
  beforeEach(() => {
    ...
    const ycDirectory = path.join(temporaryDirectory, ".yourcoder")
    fs.mkdirSync(ycDirectory, { recursive: true })
    configPath = path.join(ycDirectory, "yourcoder.json")
  })
  ...
  test("returns unpinned for bare package name", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["yourcoder-plugin"] }))
  ...
  test("returns unpinned for latest dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["yourcoder-plugin@latest"] }))
  ...
  test("returns unpinned for beta dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["yourcoder-plugin@beta"] }))
  ...
  test("returns pinned for explicit semver", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["yourcoder-plugin@3.5.2"] }))
