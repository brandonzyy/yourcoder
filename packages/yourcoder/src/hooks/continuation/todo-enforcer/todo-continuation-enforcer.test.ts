...
describe("todo-continuation-enforcer", () => {
  ...
  test("should extract model from assistant message with flat modelID/providerID", async () => {
    ...
    const mockMessagesWithAssistant = [
      { info: { id: "msg-1", role: "user", agent: "yc", model: { providerID: "openai", modelID: "gpt-5.4" } } },
      { info: { id: "msg-2", role: "assistant", agent: "yc", modelID: "gpt-5.4", providerID: "openai" } },
    ]
  ...
  test("should skip compaction agent messages when resolving agent info", async () => {
    // given - session where last message is from compaction agent but previous was Yac
    const sessionID = "main-compaction-filter"
    setMainSession(sessionID)

    const mockMessagesWithCompaction = [
      { info: { id: "msg-1", role: "user", agent: "yc", model: { providerID: "anthropic", modelID: "claude-sonnet-4-6" } } },
      { info: { id: "msg-2", role: "assistant", agent: "yc", modelID: "claude-sonnet-4-6", providerID: "anthropic" } },
    ...
    const mockInput = {
     ...
     // then - continuation uses Yac (skipped compaction agent)
     expect(promptCalls.length).toBe(1)
    expect(promptCalls[0].agent).toBe("yc")
  })
