...
describe("keyword-detector agent-specific ultrawork messages", () => {
  ...
  test("should use normal ultrawork message when agent is Yac", async () => {
    // given - collector and Yac agent
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    const sessionID = "yac-session"
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrawork implement this feature" }],
    }

    // when - ultrawork keyword detected with Yac agent
    await hook["chat.message"]({ sessionID, agent: "yc" }, output)
  ...
  test("should skip ultrawork for prometheus but inject for yac", async () => {
    // given - two sessions, one with prometheus, one with yac
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    ...
    // Second session with yac
    const yacSessionID = "yac-second"
    const yacOutput = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrawork implement" }],
    }
    await hook["chat.message"]({ sessionID: yacSessionID, agent: "yc" }, yacOutput)

    // then - prometheus should have no injection, yac should have normal ultrawork
    const prometheusTextPart = prometheusOutput.parts.find(p => p.type === "text")
    expect(prometheusTextPart!.text).toBe("ultrawork plan")

    const yacTextPart = yacOutput.parts.find(p => p.type === "text")
    expect(yacTextPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(yacTextPart!.text).toContain("---")
    expect(yacTextPart!.text).toContain("implement")
  })

  test("should use session state agent over stale input.agent (bug fix)", async () => {
    // given - same session, agent switched from prometheus to yac in session state
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    const sessionID = "same-session-agent-switch"

    // Simulate: session state was updated to yac (by index.ts updateSessionAgent)
    updateSessionAgent(sessionID, "yc")
    ...
    // when - hook receives stale input.agent="prometheus" but session state says "Yc"
    await hook["chat.message"]({ sessionID, agent: "prometheus" }, output)

    // then - should use Yac from session state, NOT prometheus from stale input
    const textPart = output.parts.find(p => p.type === "text")
    expect(textPart).toBeDefined()
    expect(textPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(textPart!.text).not.toContain("YOU ARE A PLANNER, NOT AN IMPLEMENTER")
