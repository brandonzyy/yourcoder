...
describe("category-skill-reminder hook", () => {
  ...
  describe("target agent detection", () => {
    test("should inject reminder for yc agent after 3 tool calls", async () => {
      // given - yc agent session with multiple tool calls
    ...
    test("should inject reminder for coderhand agent", async () => {
      // given - coderhand agent session
      const hook = createHook()
      const sessionID = "junior-session"
      updateSessionAgent(sessionID, "coderhand")
    ...
    test("should NOT inject reminder for non-target agents", async () => {
      // given - codersearch agent session (not a target)
      const hook = createHook()
      const sessionID = "codersearch-session"
      updateSessionAgent(sessionID, "codersearch")
    ...
    test("should detect agent from input.agent when session state is empty", async () => {
      ...
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "1", agent: "Yc" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "2", agent: "Yc" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "3", agent: "Yc" }, output)
  ...
  describe("delegation tool tracking", () => {
    ...
    test("should NOT inject reminder if call_omo_agent is used", async () => {
      // given - yc agent that uses call_omo_agent
      const hook = createHook()
    ...
    test("should NOT inject reminder if task tool is used", async () => {
      // given - yc agent that uses task tool
      const hook = createHook()
  ...
  describe("tool call counting", () => {
    test("should NOT inject reminder before 3 tool calls", async () => {
      // given - yc agent with only 2 tool calls
      const hook = createHook()
    ...
    test("should only count delegatable work tools", async () => {
      // given - yc agent with mixed tool calls
      const hook = createHook()
  ...
  describe("event handling", () => {
    test("should reset state on session.deleted event", async () => {
      // given - yc agent with reminder already shown
      const hook = createHook()
  ...
  describe("case insensitivity", () => {
    test("should handle tool names case-insensitively", async () => {
      // given - yc agent with mixed case tool names
      const hook = createHook()
    ...
    test("should handle delegation tool names case-insensitively", async () => {
      // given - yc agent using TASK in uppercase
      const hook = createHook()
