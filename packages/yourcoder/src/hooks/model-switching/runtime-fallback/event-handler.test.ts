...
function deps(
  ...
  const dep: Parameters<typeof createEventHandler>[0] = {
    ctx: {
      directory: "/tmp/project",
      client: {
        session: {
          messages: mock(async () => ({ data: [] })) as never,
          promptAsync: mock(async () => ({})) as never,
          abort: mock(async () => ({})) as never,
        } as unknown as Parameters<typeof createEventHandler>[0]["ctx"]["client"]["session"],
        tui: {
          showToast: mock(async () => ({})) as never,
        } as unknown as Parameters<typeof createEventHandler>[0]["ctx"]["client"]["tui"],
      } as unknown as Parameters<typeof createEventHandler>[0]["ctx"]["client"],
    } as unknown as Parameters<typeof createEventHandler>[0]["ctx"],
...
function helpers() {
  const help: Parameters<typeof createEventHandler>[1] = {
