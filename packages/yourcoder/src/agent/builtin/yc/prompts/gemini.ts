/**
 * Gemini-specific overlay sections for Yc prompt.
 *
 ...
 * in the dynamic Yc prompt to counter these tendencies.
 */
...
| \`Task\` | ANY non-trivial implementation. Research via codereye/codersearch. | ✅ Fire multiple in background |
...
1. Fire codereye agents to find relevant files — pass exact paths to Junior
...
**ALL code search → codereye agent.** Never use grep/glob/ast_grep directly.
