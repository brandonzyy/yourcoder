...
export namespace Agent {
  ...
  const state = Instance.state(async () => {
      ...
      plan: {
        ...
        permission: PermissionNext.merge(
          ...
          PermissionNext.fromConfig({
            ...
            edit: {
              "*": "deny",
              [path.join(".yourcoder", "plans", "*.md")]: "allow",
    ...
    try {
      ...
      // When yac is loaded, make it the default and demote build
      if (builtinAgents.yac && result.build) {
  ...
  export async function list() {
    ...
    return pipe(
      await state(),
      values(),
      sortBy([(x) => (cfg.default_agent ? x.name === cfg.default_agent : x.name === "yc" || x.name === "build"), "desc"]),
    )
  }
