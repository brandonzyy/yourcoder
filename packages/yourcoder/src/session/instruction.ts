...
function globalFiles() {
  ...
  if (Flag.YC_CONFIG_DIR) {
    files.push(path.join(Flag.YC_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.YC_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.YC_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no YC_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.YC_CONFIG_DIR, Flag.YC_CONFIG_DIR).catch(() => [])
}
