...
import { isOpenCodeVersionAtLeast, YC_SQLITE_VERSION } from "./opencode-version"
...
export function isSqliteBackend(): boolean {
  ...
  const check = (): boolean => {
    const versionOk = isOpenCodeVersionAtLeast(YC_SQLITE_VERSION)
    const dbPath = join(getDataDir(), "yourcoder", "opencode.db")
