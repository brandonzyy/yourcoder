...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.YC_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.YC_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.YC_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".yourcoder") && dir !== Flag.YC_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old yourcoder.json shape) still get their settings applied.
    const normalized = (() => {
