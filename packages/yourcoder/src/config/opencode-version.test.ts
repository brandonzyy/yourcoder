...
import {
  ...
  setVersionCache,
  MINIMUM_YC_VERSION,
  YC_NATIVE_AGENTS_INJECTION_VERSION,
...
describe("opencode-version", () => {
  ...
  describe("MINIMUM_YC_VERSION", () => {
    test("is set to 1.1.1", () => {
      expect(MINIMUM_YC_VERSION).toBe("1.1.1")
    })
  })

  describe("YC_NATIVE_AGENTS_INJECTION_VERSION", () => {
    test("is set to 1.1.37", () => {
      // given the native agents injection version constant
      // when exported
      // then it should be 1.1.37 (PR #10678)
      expect(YC_NATIVE_AGENTS_INJECTION_VERSION).toBe("1.1.37")
    })
    ...
    test("version detection works correctly with native agents version", () => {
      ...
      const result = isOpenCodeVersionAtLeast(YC_NATIVE_AGENTS_INJECTION_VERSION)
