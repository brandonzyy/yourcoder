...
export function getOpenCodeStorageDir(): string {
  return path.join(getDataDir(), "yourcoder", "storage")
}
...
/**
 * Returns the yourcoder-plugin cache directory.
 * All platforms: ~/.cache/yourcoder-plugin
 */
export function getPluginCacheDir(): string {
  return path.join(getCacheDir(), "yourcoder-plugin")
}
...
export function getOpenCodeCacheDir(): string {
  return path.join(getCacheDir(), "yourcoder")
}
