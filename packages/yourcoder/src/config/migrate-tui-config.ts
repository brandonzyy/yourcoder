...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from yourcoder.json files
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.YC_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("yourcoder", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "yourcoder")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "yourcoder"))
  }
  if (Flag.YC_CONFIG) files.push(Flag.YC_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "yourcoder"))
