...
describe("opencode-config-dir", () => {
  ...
  beforeEach(() => {
    ...
    originalEnv = {
      ...
      YC_CONFIG_DIR: process.env.YC_CONFIG_DIR,
    }
  })
  ...
  describe("YC_CONFIG_DIR environment variable", () => {
    test("returns YC_CONFIG_DIR when env var is set", () => {
      // given YC_CONFIG_DIR is set to a custom path
      process.env.YC_CONFIG_DIR = "/custom/opencode/path"
      Object.defineProperty(process, "platform", { value: "linux" })

      // when getOpenCodeConfigDir is called with binary="opencode"
      const result = getOpenCodeConfigDir({ binary: "yourcoder", version: "1.0.200" })
    ...
    test("falls back to default when env var is not set", () => {
      // given YC_CONFIG_DIR is not set, platform is Linux
    ...
    test("falls back to default when env var is empty string", () => {
      // given YC_CONFIG_DIR is set to empty string
      process.env.YC_CONFIG_DIR = ""
      delete process.env.XDG_CONFIG_HOME
      Object.defineProperty(process, "platform", { value: "linux" })
    ...
    test("falls back to default when env var is whitespace only", () => {
      // given YC_CONFIG_DIR is set to whitespace only
      process.env.YC_CONFIG_DIR = "   "
      delete process.env.XDG_CONFIG_HOME
      Object.defineProperty(process, "platform", { value: "linux" })
    ...
    test("resolves relative path to absolute path", () => {
      // given YC_CONFIG_DIR is set to a relative path
      process.env.YC_CONFIG_DIR = "./my-opencode-config"
      Object.defineProperty(process, "platform", { value: "linux" })
    ...
    test("YC_CONFIG_DIR takes priority over XDG_CONFIG_HOME", () => {
      // given both YC_CONFIG_DIR and XDG_CONFIG_HOME are set
      process.env.YC_CONFIG_DIR = "/custom/opencode/path"
      ...
      // then YC_CONFIG_DIR takes priority
      expect(result).toBe(resolve("/custom/opencode/path"))
    })
  })
  ...
  describe("getOpenCodeConfigDir", () => {
    describe("for opencode CLI binary", () => {
      ...
      test("returns $XDG_CONFIG_HOME/opencode on Linux when XDG_CONFIG_HOME is set", () => {
        ...
        expect(result).toBe(join(process.env.XDG_CONFIG_HOME!, "opencode"))
      })
      ...
      test("returns ~/.config/opencode on Windows by default", () => {
        ...
        // when getOpenCodeConfigDir is called with binary="opencode"
        const result = getOpenCodeConfigDir({ binary: "yourcoder", version: "1.0.200", checkExisting: false })
  ...
  describe("getOpenCodeConfigPaths", () => {
    test("returns all config paths for CLI binary", () => {
      ...
      const paths = getOpenCodeConfigPaths({ binary: "yourcoder", version: "1.0.200" })

      // then returns all expected paths
      const expectedDir = join(homedir(), ".config", "opencode")
      expect(paths.configDir).toBe(expectedDir)
      expect(paths.configJson).toBe(join(expectedDir, "yourcoder.json"))
      expect(paths.configJsonc).toBe(join(expectedDir, "yourcoder.jsonc"))
      expect(paths.packageJson).toBe(join(expectedDir, "package.json"))
      expect(paths.pluginConfig).toBe(join(expectedDir, "yourcoder-plugin.json"))
    })
  ...
  describe("detectExistingConfigDir", () => {
    ...
    test("includes YC_CONFIG_DIR in search locations when set", () => {
      // given YC_CONFIG_DIR is set to a custom path
      process.env.YC_CONFIG_DIR = "/custom/opencode/path"
      Object.defineProperty(process, "platform", { value: "linux" })
      delete process.env.XDG_CONFIG_HOME
