...
import { ModelSchema, InfoSchema } from "./model-schema"
import { BUNDLED_PROVIDERS } from "./bundled-providers"
import { CUSTOM_LOADERS, type CustomModelLoader } from "./custom-loaders"
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.YC_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
