...
import type { Hooks } from "../../plugin/sdk"
...
export const AuthLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "yourcoder auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in yourcoder.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via yourcoder.json options (profile, region, endpoint) or\n" +
