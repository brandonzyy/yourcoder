...
import { createYcClient, type Message, type YcClient, type ToolPart } from "@yourcoder/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run yourcoder with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
  ...
  handler: async (args) => {
    ...
    async function session(sdk: YcClient) {
    ...
    async function share(sdk: YcClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.YC_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: YcClient) {
    ...
    if (args.attach) {
      const sdk = createYcClient({ baseUrl: args.attach, directory })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createYcClient({ baseUrl: "http://opencode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
