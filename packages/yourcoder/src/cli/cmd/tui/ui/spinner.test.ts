...
describe("spinner helpers", () => {
  ...
  it("creates a color generator that brightens the active position", () => {
    ...
    const head = lead instanceof RGBA ? lead : RGBA.fromHex(lead as string)
    const trail = tail instanceof RGBA ? tail : RGBA.fromHex(tail as string)
    expect(lead).toBeInstanceOf(RGBA)
    expect(tail).toBeInstanceOf(RGBA)
    expect(head.a).toBeGreaterThan(trail.a)
  })
})
