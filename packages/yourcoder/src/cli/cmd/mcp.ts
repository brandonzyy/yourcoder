    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: yc mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in yourcoder.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .yourcoder/ subdirectory too)
  const candidates = [path.join(baseDir, "yourcoder.json"), path.join(baseDir, "yourcoder.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".yourcoder", "yourcoder.json"), path.join(baseDir, ".yourcoder", "yourcoder.jsonc"))
  }
  ...
  // Default to yourcoder.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., yc x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "yourcoder-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
