...
import { resolveSkillContent, resolveMultipleSkills, resolveSkillContentAsync, resolveMultipleSkillsAsync } from "./skill-template-resolver"
...
beforeEach(() => {
	...
	process.env.YC_CONFIG_DIR = testConfigDir
})
...
describe("resolveMultipleSkillsAsync", () => {
	...
	it("should NOT inject watermark when both options are disabled", async () => {
		...
		expect(gitMasterContent).not.toContain("Co-authored-by: Yac")
	})
	...
	it("should inject watermark when enabled (default)", async () => {
		...
		expect(result.resolved.size).toBe(1)
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).toContain("Ultraworked with [Yac]")
		expect(gitMasterContent).toContain("Co-authored-by: Yac")
	})
	...
	it("should inject only co-author when footer is disabled", async () => {
		...
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).not.toContain("Ultraworked with [Yac]")
		expect(gitMasterContent).toContain("Co-authored-by: Yac")
	})
	...
	it("should use default Yac footer when commit_footer is boolean true", async () => {
		...
		// then: default Yac footer is injected
		const gitMasterContent = result.resolved.get("git-master")
		expect(gitMasterContent).toContain("Ultraworked with [Yac]")
	})
