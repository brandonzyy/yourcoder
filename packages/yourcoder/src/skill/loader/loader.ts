...
export async function loadOpencodeProjectSkills(directory?: string): Promise<Record<string, CommandDefinition>> {
  const ycProjectDir = join(directory ?? process.cwd(), ".yourcoder", "skills")
  const skills = await loadSkillsFromDir({ skillsDir: ycProjectDir, scope: "opencode-project" })
  return skillsToCommandDefinitionRecord(skills)
}
...
export async function discoverOpencodeProjectSkills(directory?: string): Promise<LoadedSkill[]> {
  const ycProjectDir = join(directory ?? process.cwd(), ".yourcoder", "skills")
  return loadSkillsFromDir({ skillsDir: ycProjectDir, scope: "opencode-project" })
}
