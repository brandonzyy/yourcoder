import type { GitMasterConfig } from "../../config/plugin-schema"
...
export function injectGitMasterConfig(template: string, config?: GitMasterConfig): string {
	...
	sections.push("")
	sections.push("Add YourCoder attribution to EVERY commit:")
	sections.push("")
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Yac <yac@yourcoder.com>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Yac <yac@yourcoder.com>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			"git commit -m \"{Commit Message}\" -m \"Co-authored-by: Yac <yac@yourcoder.com>\""
		)
		sections.push("```")
	}
