...
const SKILLS_DIR = join(TEST_DIR, ".yourcoder", "skills")
...
`
  ...
  describe("deduplication", () => {
    it("deduplicates skills by name across scopes, keeping higher priority (opencode-project > opencode > project)", async () => {
      const originalCwd = process.cwd()
      const originalOpenCodeConfigDir = process.env.YC_CONFIG_DIR
      const originalClaudeConfigDir = process.env.CLAUDE_CONFIG_DIR

      // given: same skill name in multiple scopes
      const ycProjectSkillsDir = join(TEST_DIR, ".yourcoder", "skills")
      ...
      process.env.YC_CONFIG_DIR = opencodeConfigDir
      process.env.CLAUDE_CONFIG_DIR = join(TEST_DIR, "claude-user")

      mkdirSync(join(ycProjectSkillsDir, "duplicate-skill"), { recursive: true })
      ...
      writeFileSync(
        join(ycProjectSkillsDir, "duplicate-skill", "SKILL.md"),
        `---
...
`
      ...
      try {
        ...
        if (originalOpenCodeConfigDir === undefined) {
          ...
          process.env.YC_CONFIG_DIR = originalOpenCodeConfigDir
        }
...
`
    ...
    it("returns no duplicates from discoverSkills", async () => {
      ...
      process.env.YC_CONFIG_DIR = join(TEST_DIR, "opencode-global")
