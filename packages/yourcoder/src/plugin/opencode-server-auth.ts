 ...
 *
 * @returns Basic Auth header string, or undefined if YC_SERVER_PASSWORD is not set
 */
export function getServerBasicAuthHeader(): string | undefined {
  const password = process.env.YC_SERVER_PASSWORD
  if (!password) {
    return undefined
  }

  const username = process.env.YC_SERVER_USERNAME ?? "opencode"
 ...
 * @throws {Error} If YC_SERVER_PASSWORD is set but client structure is incompatible
 */
export function injectServerAuthIntoClient(client: unknown): void {
  ...
  try {
    ...
    if (internal) {
      ...
      if (!injected) {
        log("[opencode-server-auth] YC_SERVER_PASSWORD is set but SDK client structure is incompatible", {
    ...
    if (!injected) {
      log("[opencode-server-auth] YC_SERVER_PASSWORD is set but no compatible SDK client found")
    }
  } catch (error) {
