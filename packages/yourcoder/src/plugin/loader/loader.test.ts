...
describe("loadAllPluginComponents", () => {
  it("aggregates components from every plugin loader", async () => {
    ...
    const commands = { "demo:build": { name: "demo:build", description: "cmd", template: "cmd" } }
    const skills = { "demo:skill": { name: "demo:skill", description: "skill", template: "skill" } }
    const agents = { "demo:agent": { description: "agent", mode: "subagent" as const, prompt: "hi" } }
    const mcpServers = { "demo:mcp": { type: "remote" as const, url: "https://mcp.example.com", enabled: true } }
