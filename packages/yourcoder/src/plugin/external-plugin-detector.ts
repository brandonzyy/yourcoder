/**
 * Detects external plugins that may conflict with yourcoder-plugin features.
...
/**
 * Known notification plugins that conflict with yourcoder-plugin's session-notification.
...
function getConfigPaths(directory: string): string[] {
  ...
  const paths = [
    ...
    path.join(crossPlatformDir, "opencode", "yourcoder.json"),
    path.join(crossPlatformDir, "opencode", "yourcoder.jsonc"),
  ]

  if (process.platform === "win32") {
    const appdataDir = getWindowsAppdataDir()
    if (appdataDir) {
      paths.push(path.join(appdataDir, "opencode", "yourcoder.json"))
      paths.push(path.join(appdataDir, "opencode", "yourcoder.jsonc"))
    }
  }
...
Both yourcoder-plugin and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   yourcoder-plugin's session-notification has been auto-disabled.

   To use yourcoder-plugin's notifications instead, either:
   1. Remove ${pluginName} from your yourcoder.json plugins
   2. Or set "notification": { "force_enable": true } in yourcoder-plugin.json`
}
