...
describe("yac-task", () => {
  ...
  describe("category delegation config validation", () => {
    test("fills subagent_type as coderhand when category is provided without subagent_type", async () => {
      // given
      const { createDelegateTask } = require("./tools")
      ...
      const mockManager = {
       ...
       // then
       expect(args.subagent_type).toBe("CoderHand")
    }, { timeout: 10000 })

    test("category overrides subagent_type and still maps to coderhand", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const { createDelegateTask } = require("./tools")
       ...
       const mockManager = { launch: async () => ({ id: "task-123", status: "pending", description: "Test task", agent: "coderhand", sessionID: "test-session" }) }
       const mockClient = {
         app: { agents: async () => ({ data: [] }) },
       ...
       expect(result).not.toContain("yourcoder-plugin requires a default model")
    }, { timeout: 10000 })
  ...
  test("sync continuation preserves variant from previous session message", async () => {
    ...
    const tool = createDelegateTask({
      manager: { resume: async () => ({ id: "task-var", sessionID: "ses_var_test", description: "Variant test", agent: "coderhand", status: "running" }) },
      client: mockClient,
    })
    ...
    expect(callArgs.body.agent).toBe("coderhand")
  ...
  describe("category model resolution fallback", () => {
    test("category uses resolved.model when connectedProvidersCache is null and availableModels is empty", async () => {
      ...
      // NO userCategories override, NO coderhandModel
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
    ...
    test("coderhand model override takes precedence over category model", async () => {
      // given - coderhand override model differs from category default
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        coderhandModel: "anthropic/claude-sonnet-4-6",
        connectedProvidersOverride: TEST_CONNECTED_PROVIDERS,
        availableModelsOverride: createTestAvailableModels(),
      })
    ...
    test("explicit category model takes precedence over coderhand model", async () => {
      // given - explicit category model differs from coderhand override
      const { createDelegateTask } = require("./tools")
      let launchInput: any
    ...
    test("coderhand model override works with quick category (#1295)", async () => {
      // given - user configures agents.coderhand.model but uses quick category
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      // then - coderhand override model should be used, not category default
      expect(launchInput.model.providerID).toBe("anthropic")
      expect(launchInput.model.modelID).toBe("claude-sonnet-4-6")
    })

    test("coderhand model override works with user-defined category (#1295)", async () => {
      ...
      // then - coderhand override model should be used as fallback
      expect(launchInput.model.providerID).toBe("openai")
  ...
  describe("plan family mutual delegation block", () => {
    ...
    test("yac CAN delegate to plan (not in plan family)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "yc", abort: new AbortController().signal }
      )
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    ...
    test("agentOverrides model takes priority over matchedAgent.model (#1357)", async () => {
      // given - user configured oracle to use a specific model in yourcoder-plugin.json
      const { createDelegateTask } = require("./tools")
      let promptBody: any
  ...
  describe("subagent task permission", () => {
    test("plan subagent should have task permission enabled", async () => {
      //#given - yac delegates to plan agent
      const { createDelegateTask } = require("./tools")
      let promptBody: any
    ...
    test("non-plan subagent should NOT have task permission", async () => {
      //#given - yac delegates to oracle (non-plan)
      const { createDelegateTask } = require("./tools")
      let promptBody: any
  ...
  describe("session title and metadata format (OpenCode compatibility)", () => {
    test("sync session title follows OpenCode format: '{description} (@{agent} subagent)'", async () => {
      ...
      expect(createBody.title).toBe("Implement feature X (@CoderHand subagent)")
    }, { timeout: 10000 })
    ...
    test("background task output includes <task_metadata> block with session_id", async () => {
      ...
      const mockManager = {
       ...
       const tool = createDelegateTask({
         ...
         client: mockClient,
         userCategories: {
           "coderhand": { model: "anthropic/claude-sonnet-4-6" },
         },
       })
