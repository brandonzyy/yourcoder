...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "../plugin/sdk"
import z from "zod"
...
export namespace ToolRegistry {
  ...
  async function all(): Promise<Tool.Info[]> {
    ...
    const question = ["app", "cli", "desktop"].includes(Flag.YC_CLIENT) || Flag.YC_ENABLE_QUESTION_TOOL
    ...
    return [
      ...
      SkillTool,
      ApplyPatchTool,
      ...(Flag.YC_EXPERIMENTAL_LSP_TOOL ? [LspTool] : []),
      ...(config.experimental?.batch_tool === true ? [BatchTool] : []),
      ...(Flag.YC_EXPERIMENTAL_PLAN_MODE && Flag.YC_CLIENT === "cli" ? [PlanExitTool] : []),
      ...custom,
    ]
  }
      ...
      tools
        .filter((t) => {
          ...
          if (t.id === "codesearch" || t.id === "websearch") {
            return model.providerID === "opencode" || Flag.YC_ENABLE_EXA
          }
