...
export namespace Database {
  export const Path = iife(() => {
    ...
    if (["latest", "beta"].includes(channel) || Flag.YC_DISABLE_CHANNEL_DB)
  ...
  export const Client = lazy(() => {
    ...
    const entries =
      typeof YC_MIGRATIONS !== "undefined"
        ? YC_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof YC_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
      if (Flag.YC_SKIP_MIGRATIONS) {
